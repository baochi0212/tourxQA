...
def train_QA(model, optimizer, scheduler, train_dataloader, total_steps, epochs, device, tokenizer, val_dataloader=None, evaluation=False, overfit_batch=False):
    """
    -Set the val to None: if don't wanna keep track of validation.
    -Overfit one batch: for check sanity of model
    -init model before training
    ...
    for epoch_i in range(epochs):
        ...
        if evaluation == True:
            ...
            val_loss, val_accuracy, EM_score, F1_score, EM1_score, F11_score = evaluate_QA(model, val_dataloader, tokenizer=tokenizer, device=device)

            # Print performance over the entire training data
            time_elapsed = time.time() - t0_epoch
            # , {train_accuracy:^9.2f}
            # print(f"{epoch_i + 1:^7} | {'-':^7} | {avg_train_loss:^12.6f}| {val_loss:^10.6f} | {val_accuracy:^9.2f} | {time_elapsed:^9.2f}")
            print(f"Loss: {val_loss}, EM: {EM_score} and F1: {F1_score} and EM1: {EM1_score} and F1_1: {F11_score}")
        #     print("-"*70)
        # print("\n")
    print("Training complete!")
...
if __name__ == '__main__':
    ...
    train_QA(model.to(device), optimizer, scheduler, train_loader, total_steps, epochs, tokenizer=tokenizer, device=device, val_dataloader=val_loader, evaluation=True, overfit_batch=False)
